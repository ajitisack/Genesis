...
def getprice(rpt=80):
    ...
    df['qty']        = df.apply(lambda x: round(rpt/(x['ltp']*0.005), 0), axis = 1)
    df['slb']        = df['ltp'].apply(lambda x: round(x-(x*0.005),0))
    df['sls']        = df['ltp'].apply(lambda x: round(x+(x*0.005),0))
    ...
    return df_symbols, df_indices1, df_indices2
...
def layout():
    body = dbc.Container([
        ...
        , dbc.Row([
            dbc.Col(html.Div(pricetbl('tbl1', getprice()[0]), id='stock-data1'), width=6),
            dbc.Col(html.Div(pricetbl('tbl2', getprice()[0]), id='stock-data2'), width=6),
          ])
        , dcc.Interval(
                    id = 'interval-component',
                    interval = 5 * 1000, # in milliseconds ie, 1 x 1000 means 1 sec
                    n_intervals = 0
        )
    ], fluid = True)
...
def update_table(x):
    df, df_indices1, df_indices2 = getprice()
    ...
    return pricetbl('tbl1', dfu), pricetbl('tbl2', dfd), indicestbl('tbl3', df_indices1), indicestbl('tbl4', df_indices2)
